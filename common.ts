...
import { readableStreamToString, ShutterstockError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new ShutterstockError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new ShutterstockError(e, e.response?.data)
                }
                if (innerError instanceof ShutterstockError) {
                    // Got "ShutterstockError" from the above try block
                    throw innerError;
                }
